...
function cadastroAdministrador() {
    ...
    .then(res => res.json())
    .then(data => {
        alert('Administrador adicionado!', data);
...
function cadastroVendedor() {
    ...
    .then(res => res.json())
    .then(data => {
        alert('Vendedor adicionado!'); // Adicionado com sucesso
...
function showListAdmin(){
    ...
    function listaAdministradores(data) {
        ...
        data.forEach((item) => {
            ...
            linha.appendChild(buttonDelet);
            
            table.appendChild(linha); // Adiciona a linha à tabela
        });
    }
}
    ...
    function listarcardProdutos(data) {
        ...
        data.forEach((item) => {
            ...
            buttonEdit.onclick = function(){
                ...
                console.log('ID da linha:', id); // Recupera o ID da linha
                console.log('Editando', id);
                editProduto( id, item.metadata.nome, item.metadata.preco, item.metadata.categoria, item.metadata.quantidade, imgSrc.src); // Passa o ID para a função de edição
...
function editarAdm(id, nome, registro) {
    ...
    document.getElementById('saveUpdated').onclick = function() {
        ...
        fetch(`http://localhost:3000/admin/administrador/${id}`, { // Inclua o ID na URL para atualizar o recurso correto
            method: 'PATCH',
            headers: {
                'Content-Type': 'application/json'
            },
...
function editarVend(id, nome, registro) {
    ...
    document.getElementById('saveUpdatedVend').onclick = function() {
        ...
        fetch(`http://localhost:3000/vendedor/vendedor/${id}`, { // Inclua o ID na URL para atualizar o recurso correto
            method: 'PATCH',
            headers: {
                'Content-Type': 'application/json'
            },
...
function deletAdm(id){
    ...
    .then(data => {
        ...
        showListAdmin(); // Atualiza a lista após a exclusão
    })
    .catch(error => {
...
function deletVend(id){
    ...
    .then(data => {
        ...
        // Aqui você pode chamar uma função para atualizar a lista após a exclusão
        showListVendedor(); // Atualiza a lista após a exclusão
    })
    .catch(error => {
        console.error('Erro:', error);
    });
...
function deletProduto(id){
    fetch(`http://localhost:3000/produto/produto/${id}`, { // Inclua o ID na URL para atualizar o recurso correto
        method: 'DELETE',
        headers: {
            'Content-Type': 'application/json'
        }
